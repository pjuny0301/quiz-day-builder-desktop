...
public static class Win32Flow4 {
...
function SaveShot([string]$Path) {
  $handle = [Win32Flow4]::GetForegroundWindow(); $rect = New-Object Win32Flow4+RECT; [void][Win32Flow4]::GetWindowRect($handle, [ref]$rect)
...
function ClickOffset([int]$OffsetX,[int]$OffsetY) {
  ...
  [void][Win32Flow4]::SetCursorPos($x, $y)
  Start-Sleep -Milliseconds 200
  [Win32Flow4]::mouse_event(0x0002,0,0,0,[UIntPtr]::Zero)
  [Win32Flow4]::mouse_event(0x0004,0,0,0,[UIntPtr]::Zero)
}
...
try {
  ...
  SaveShot (Join-Path $OutputDir 'study-flow-3-before-click.png')
  ClickOffset 335 420
  Start-Sleep -Milliseconds 2200
  SaveShot (Join-Path $OutputDir 'study-flow-4-after-click.png')
}
finally {
